      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          <Link to="/" className="flex items-center gap-2.5 group">
            ...
            <span className="font-bold text-xl text-gray-900">Rural<span className="text-primary-500">Connect</span></span>
          </Link>
          ...
          <div className="hidden md:flex items-center gap-1">
            <Link
              ...
              className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${
                isActive('/products') ? 'bg-primary-50 text-primary-700' : 'text-gray-600 hover:text-gray-900 hover:bg-stone-100'
              }`}
            >
              Marketplace
            </Link>
            {user && dashboardPath && (
              <Link
                ...
                className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors flex items-center gap-1.5 ${
                  location.pathname.startsWith(dashboardPath) ? 'bg-primary-50 text-primary-700' : 'text-gray-600 hover:text-gray-900 hover:bg-stone-100'
                }`}
              >
            ...
            {user && profile ? (
              <div className="flex items-center gap-3">
                <div className="flex items-center gap-2 px-3 py-1.5 bg-stone-100 rounded-lg">
                  ...
                  <span className="text-xs bg-primary-100 text-primary-700 px-2 py-0.5 rounded-full font-medium">{profile.role}</span>
                </div>
                <button
              ...
              <div className="flex items-center gap-2">
                ...
                <Link
                  to="/register"
                  className="btn-premium px-4 py-2 text-sm font-medium bg-primary-500 text-white rounded-lg hover:bg-primary-600 transition-colors"
                >
                  Get started
                </Link>
              </div>
        ...
        <div className="md:hidden border-t border-stone-200 bg-white px-4 py-3 space-y-1">
          ...
          {user ? (
            ...
            <div className="pt-2 flex flex-col gap-2">
              ...
              <Link to="/register" onClick={() => setMobileOpen(false)} className="btn-premium px-4 py-2.5 text-center text-sm font-medium bg-primary-500 text-white rounded-lg hover:bg-primary-600">Get started</Link>
            </div>
          )}
        </div>
      )}
